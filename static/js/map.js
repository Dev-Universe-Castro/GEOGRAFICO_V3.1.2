        ...
        for (const filePath of geoJsonFiles) {
            try {
                ...
                if (response.ok) {
                    ...
                    const filteredData = applyStateFilterForLayer(geoData, currentStateFilter);
...
// Função para reposicionar cards analíticos
...
function showAnalyticsCard(layer) {
    ...
    if (layer.type === 'receitas') {
        ...
        `;
    } else if (layer.type === 'revendas' && layer.municipalityDetails) {
        const totalMunicipios = layer.municipalityDetails.length;
        const estadosUnicos = [...new Set(layer.municipalityDetails.map(m => m.state))];
        ...
        cardHTML += `
            ...
            <div style="margin-bottom: 15px;">
                ...
                <div style="max-height: 120px; overflow-y: auto;">
                    ${layer.municipalityDetails && layer.municipalityDetails.length > 0 ? 
                        layer.municipalityDetails.slice(0, 5).map((municipio, index) => `
                            <div style="display: flex; align-items: center; padding: 4px 0; border-bottom: 1px solid #f0f0f0;">
                                ...
                                <div style="flex-grow: 1;">
                                    <div style="font-size: 11px; font-weight: 500;">${municipio.name && municipio.name !== `Município ${municipio.code}` ? municipio.name : `Município ${municipio.code}`}</div>
                                    <div style="font-size: 10px; color: #666;">${municipio.state && municipio.state !== 'XX' ? municipio.state : 'N/A'}</div>
                                </div>
            ...
            <div style="margin-bottom: 10px;">
                ...
                <div style="max-height: 100px; overflow-y: auto; background: #f8f9fa; border-radius: 4px; padding: 8px;">
                    ${layer.municipalityDetails && layer.municipalityDetails.length > 0 ? 
                        layer.municipalityDetails.map((municipio, index) => `
                            <div style="display: flex; justify-content: space-between; align-items: center; padding: 2px 0; font-size: 10px;">
                                ...
                                <span style="flex-grow: 1; margin-left: 8px;">${municipio.name && municipio.name !== `Município ${municipio.code}` ? municipio.name : `Código ${municipio.code}`} (${municipio.state && municipio.state !== 'XX' ? municipio.state : 'N/A'})</span>
                                <span style="color: ${layer.color || '#4CAF50'}; font-size: 10px; font-weight: 600;">Território</span>
                            </div>
                        `).join('') :
            ...
            revendasData.forEach(revenda => {
                ...
                panelHTML += `
                    <div class="revenda-item ${selectedClass}" 
                        ...
                        <div style="font-size: 11px; color: #666;">
                            CNPJ: ${revenda.cnpj} | CNAE: ${revenda.cnae}
                        </div>
                        <div style="font-size: 11px; color: #666; margin-top: 3px;">
                            <i class="fas fa-map-marker-alt"></i> ${revenda.municipios_count} municípios
                        </div>
                    </div>
                `;
            });
...
// Add missing loadRevendaTerritoryOnMap function
async function loadRevendaTerritoryOnMap(revenda, territoryData, multipleMode = false) {
    try {
        ...
        console.log(`Filtrados ${filteredFeatures.length} municípios para o território da revenda`);
        ...
        const territoryLayer = L.geoJSON(filteredData, {
            ...
            onEachFeature: function(feature, layer) {
                ...
                popupContent += `
                    <div style="color: ${revenda.cor}; font-weight: 600; margin: 8px 0;">
                        <i class="fas fa-store"></i> ${revenda.nome}
                    </div>
                    <div style="font-size: 12px; color: #666;">
                        CNPJ: ${revenda.cnpj}<br>
                        CNAE: ${revenda.cnae}<br>
                        Código: ${municipalityCode}
                    </div>
                `;
    ...
    try {
        ...
        const territoryLayer = L.geoJSON(filteredData, {
            ...
            onEachFeature: function(feature, layer) {
                ...
                const revendaName = layer.name.replace('Território - ', ''); // Supondo que o nome da camada seja "Território - [Nome do Vendedor]"

                popupContent += `
                    <div style="color: ${vendedor.cor}; font-weight: 600; margin: 8px 0;">
                        <i class="fas fa-user-tie"></i> ${vendedor.nome}
                    </div>
                    <div style="font-size: 12px; color: #666;">
                        E-mail: ${vendedor.email}<br>
...
window.toggleRevendaSelection = toggleRevendaSelection;
window.selectAllRevendas = selectAllRevendas;
window.clearAllRevendasSelection = clearAllRevendasSelection;
...
window.toggleVendedorSelection = toggleVendedorSelection;
window.selectAllVendedores = selectAllVendedores;
window.clearAllVendedoresSelection = clearAllVendedoresSelection;
...
function setupFeaturePopupForLayer(feature, mapFeature, layer) {
    // Try multiple ways to get municipality info from GeoJSON
    const municipalityCode = feature.properties.GEOCODIGO || feature.properties.CD_MUN || feature.properties.cd_geocmu || feature.properties.geocodigo;
    const municipalityName = feature.properties.NOME || feature.properties.NM_MUN || feature.properties.nm_mun || feature.properties.nome || 'Nome não disponível';
    const stateUF = feature.properties.UF || feature.properties.SIGLA_UF || feature.properties.uf;
...
async function loadReceitasLayerForLayer(layer) {
        ...
        async function loadMunicipalityBoundariesForRevendasLayer(layer, layerData, minMax) {
            try {
                ...
                console.log(`Tentando carregar: /static/data/brazil_municipalities_all.geojson`);
                const response = await fetch('/static/data/brazil_municipalities_all.geojson');
                ...
                const geoData = await response.json();
                console.log(`GeoJSON carregado com sucesso: /static/data/brazil_municipalities_all.geojson, ${geoData.features.length} municípios`);
                ...
                const filteredFeatures = geoData.features.filter(feature => {
                    ...
                    if (isInTerritory) {
                        console.log(`Município encontrado no território: ${municipalityCode} - ${feature.properties.NOME || feature.properties.NM_MUN || feature.properties.nm_mun}`);
                    }
                ...
                if (filteredFeatures.length === 0) {
                    ...
                    alert('Nenhum município encontrado no território da revenda. Verifique os códigos dos municípios.');
                    return;
                }
...
// Add missing loadAllDataSources function
function loadAllDataSources() {
    console.log('Loading all data sources...');
    // This function can be expanded to load initial data if needed
    // For now, it's a placeholder to prevent the "function not found" error
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        if (typeof loadAllDataSources === 'function') {
            loadAllDataSources();
            console.log('Data sources loading started');
        } else {
            console.error('loadAllDataSources function not found');
        }
    } catch (error) {
